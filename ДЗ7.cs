//ВНЕСТИ СЮДА РЕШЕНИЕ ПЕРВОЙ ЗАДАЧИ С НОУТБУКА, Т.К. ОНО РАБОТАЕТ.
...
void ShowCreateNewArray(int[,] array) 
{
     for (int i = 0; i < array.GetLength(0); i++) // такое ли имя у массива? 
...
void ShowResult(int[,] array)
{
    for (int i = 0; i < myMassive.GetLength(0); i++)
        for (int j = 0; j < myMassive.GetLength(1); j++)
            ...
            int C = A / 10;
            {
                if ( C == i && B == J)  Console.WriteLine("Ваше числоЭ + array[i,j]);
            }
            else Console.WriteLine("Такого числа не существует в массиве");
}
Console.Write("Введите количество строк: ");
int a = Convert.ToInt32(Console.ReadLine());
Console.Write("Введите количество столбцов: ");
...
// Задача 52 ВАРИАНТ 2
/*
int[,] Create2DigitsArray(int rows, int columns, int min, int max) // создали рандомный целочисленный массив
{
    int[,] myMassive = new int [rows, columns];
        for (int i = 0; i < myMassive.GetLength(0); i++)
            for (int j = 0; j < myMassive.GetLength(1); j++)
...
double Mean(int[,] array) // нахождение ср.ар по столбцу (где i = 0)
{
    for (int i = 0; i < array.GetLength(0); i++) 
        for (int j = 0; j < array.GetLength(1); j++) 
        sum = sum + array [0; rows - 1];
        mean = sum / rows;
    return mean;
}
void ShowCreate2DigitsArray(int[,] array) // вызов метода генерации рандомного массива 
